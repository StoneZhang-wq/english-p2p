...
| POST | `/api/admin/themes/:id/commit-generated-pack` | Body：`{ direction, pack, generation_id? }`。将预览 `pack`（含 `practice_kit`）通过服务端校验后**写入 themes**（默认保留当前行 `cover_url`）；若带 `generation_id`，会将对应生成记录标记为 `applied`。 |
